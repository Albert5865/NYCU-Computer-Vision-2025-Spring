...
class PromptTrainDataset(Dataset):
    ...
    def _init_ids(self):
        ...
        if 'dehaze' in self.de_type:
            self._init_hazy_ids()
        if 'desnow' in self.de_type:  # Add check for desnow
            self._init_snow_ids()
    ...
    def _init_rs_ids(self):
        ...
        self.rs_ids = rs_ids * 1   # Adjust multiplier to balance dataset size (was 120, reduced for consistency with snow)
        random.shuffle(self.rs_ids)
        self.num_rl = len(self.rs_ids)
        print("Total Rainy Ids : {}".format(self.num_rl))

    def _init_snow_ids(self):
        # Load snow data directly from the paths specified in main.py
        degraded_path = "data/train/degraded/"
        clean_path = "data/train/clean/"
        ...
        self.snow_ids = snow_ids * 1  # Adjust multiplier to balance dataset size
    ...
    def _merge_ids(self):
        ...
        if "desnow" in self.de_type:  # Add snow IDs
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            # Denoising cases remain unchanged
            if de_id == 0:
...
class TestSpecificDataset(Dataset):
    ...
    def _init_clean_ids(self, root):
        ...
        print("Total Images : {}".format(len(self.degraded_ids)))
